...
function digitPerkalianMinimum(angka) {
  ...
  for (let i = 1; i <= angka; i++) {
    if (angka % i === 0) {
      let partner = angka / i;
      himNumb.push(i.toString() + partner.toString());
    }
  }
  let terKecil = himNumb[0].length;
  for (let i = 1; i < himNumb.length; i++) {
    if (himNumb[i].length < terKecil) {
      terKecil = himNumb[i].length
    }
  }
  return terKecil
}
